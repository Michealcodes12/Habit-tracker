    ...
    <li className="group relative overflow-hidden bg-surface-container rounded-xl transition-all duration-300 hover:translate-x-1 border border-white/5 shadow-sm">
      <div className="p-5 flex items-center justify-between">
        <div className="flex items-center gap-4 cursor-pointer flex-1" onClick={() => onToggle(habit)}>
          <div className={`w-12 h-12 rounded-xl flex flex-shrink-0 items-center justify-center ${containers[styleIdx]}`}>
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="text-xs font-bold text-on-surface-variant uppercase tracking-widest">{habit.completions.length} Day{habit.completions.length !== 1 ? 's' : ''}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className={`w-12 h-12 rounded-full border-2 flex flex-shrink-0 items-center justify-center transition-all active:scale-90 ${
              isCompletedToday 
